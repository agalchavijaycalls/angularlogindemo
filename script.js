...
.factory('principal', ['$q', '$http', '$timeout', function ($q, $http, $timeout) {
  ...
  return {
    ...
    authenticate: function (identity) {
      ...
      // for this demo, we'll store the identity in localStorage. For you, it could be a cookie, sessionStorage,
      // whatever
      if (identity) {
    ...
    identity: function (force) {
      ...
      // for the sake of the demo, we'll attempt to read the identity from localStorage. the example above might be a
      // way if you use cookies or need to retrieve the latest identity from an api i put it in a timeout to illustrate
      // deferred resolution
      var self = this;
      $timeout(function () {
  ...
  return {
    authorize: function () {
      ...
      .then(function () {
        ...
        if ($rootScope.toState.data.roles && $rootScope.toState.data.roles.length > 0 && !principal.isInAnyRole(
...
.controller('HomeCtrl', ['$scope', '$state', 'principal', function ($scope, $state, principal) {
