...
.provider('loginService', function () {
  var userToken = localStorage.getItem('userToken'), errorState = 'app.error', logoutState = 'app.home';
  ...
  this.$get = function ($rootScope, $http, $q, $state) {
    ...
    var managePermissions = function () {
      ...
      $rootScope.$on('$stateChangeStart', function (event, to, toParams, from, fromParams) {
        ...
        if (to.accessLevel === undefined || to.accessLevel.bitMask & wrappedService.userRole.bitMask) {
          ...
          $state.go(errorState, {error: 'unauthorized'}, {
            location: false,
            inherit: false
          });
        }
    ...
    var wrappedService = {
      ...
      resolvePendingState: function (httpPromise) {
        var checkUser = $q.defer(), self = this, pendingState = self.pendingStateChange;
        ...
        httpPromise.then(function success(httpObj) {
          self.doneLoading = true;
          // duplicated logic from $stateChangeStart, slightly different, now we surely have the userRole informations.
          if (pendingState.to.accessLevel === undefined || pendingState.to.accessLevel.bitMask
              & self.userRole.bitMask) {
            checkUser.resolve();
          } else {
